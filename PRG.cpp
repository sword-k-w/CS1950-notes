...
void PRG::QuarterRound(std::vector<unsigned int> &state, const unsigned int &a, const unsigned int &b, const unsigned int &c, const unsigned int &d) {
...
std::vector<unsigned int> PRG::Generate(const size_t &length, const std::vector<unsigned int> &key) {
  ...
  while (key_stream.size() < length) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      QuarterRound(cur_state, 3, 15, 11, 7);
      QuarterRound(cur_state, 1, 2, 3, 0);
      QuarterRound(cur_state, 6, 7, 4, 5);
      QuarterRound(cur_state, 11, 8, 9, 10);
      QuarterRound(cur_state, 12, 13, 14, 15);
    }
